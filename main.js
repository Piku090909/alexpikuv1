...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +91853688xxxx")))

            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +91853688xxxx : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ 𝛥𝐿𝛯𝛸 𝛲𝛪𝛫𝑈 𝛭𝐷 ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: SIR-ALEX-PIKU-TECH`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: piku090909 `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: alex_piku__ `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 918536881026`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: SIR-ALEX-PIKU-TECH\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
