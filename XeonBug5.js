...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with the 𝛥𝐿𝛯𝛸 𝛲𝛪𝛫𝑈 wa.me/918536881026 💥`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
            ...
            case 'repo':
                XeonBotInc.sendMessage(m.chat, {
                    text: `repo https://github.com/piku090909/alexpikuv1
		    youtube www.youtube.com/@SIR-ALEX-PIKU-TECH`,
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `${botname}`,
                            body: `SCRIPT OF ${botname} is on YouTube www.youtube.com/@SIR-ALEX-PIKU-TECH`,
                            thumbnailUrl: 'https://telegra.ph/file/9403315158cbab1f395b1.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
...
XeonBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]}, 𝛥𝐿𝛯𝛸 𝛲𝛪𝛫𝑈 is my owner😇`, mentions: [sender]}, { quoted: repf })
}
break
            case 'sticker':
            case 'stiker':
...
case 'xkillgc': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Use ${prefix + command} https://chat.whatsapp.com/LSz84zy2oMEBcC29AhG8NK`)
await loading()
...
case 'tempban': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Example: ${prefix + command} 91|8536881026`)
if (!/|/.test(text)) return replygcxeon(`The data you provided is invalid!, Example: \n ${prefix + command} 91|8536881026`)
...
case 'gcbug' : {
if (!isPremium) return replygcxeon(mess.prem)
 if (!args[0]) return replygcxeon(`Use ${prefix+command} link\nExample ${prefix+command} https://chat.whatsapp.com/LSz84zy2oMEBcC29AhG8NK`)
await loading()
let result = args[0].split('https://chat.whatsapp.com/')[1]
...
"message": `${botname}`,
"orderTitle": " TROLLY BUG ", 
"sellerJid": "918536881026@s.whatsapp.net",
"token": "AR6z9PAvHjs9Qa7AYgBUjSEvcnOcRWycFpwieIhaMKdrhQ=="
}
}), { userJid: from, quoted:m})
...
break
            ...
            case 'help':
            case 'alive':
                let xeonmenuoh = `╭══ *❍ 𝛥𝐿𝛯𝛸-𝛲𝛪𝛫𝑈-𝛭𝐷 ❍*
┃❁ *ᴏᴡɴᴇʀ:* 𝛥𝐿𝛯𝛸 ○■□● 𝛲𝛪𝛫𝑈
┃❁ *ᴜᴘᴛɪᴍᴇ:* ${runtime(process.uptime())}
┃❁ *User:* ${pushname}
╰┈➤ ${xeonytimewisher} 😄  My bro
⚠️⚠️ ~Don't forget to subscribe~ 
*𝛥𝐿𝛯𝛸 𝛲𝛪𝛫𝑈* ~Youtube Channel~ ⚠️⚠️
_Any help: yt link http://www.youtube.com/@SIR-ALEX-PIKU-TECH_
╰═════════𝛥𝐿𝛯𝛸 𝛲𝛪𝛫𝑈════════⊷
╭═══❂ 𝗕𝗨𝗚 𝗠𝗘𝗡𝗨 ❂
┃ ❏ *xʀᴇᴀᴄᴛ <ʀᴇᴘʟʏ ᴄʜᴀᴛ>*
┃ ❏ *x <ɴᴜᴍʙᴇʀ|ᴀᴍᴏᴜɴᴛ>*
